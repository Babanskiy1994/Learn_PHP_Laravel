...
@section('title', isset($post) ? "Редактировать статью {$post->id}" : 'Добавить статью')

@section('content')
    <div class="container mx-auto px-6 py-8">
        <h3 class="text-gray-700 text-3xl font-medium">{{ isset($post) ? "Редактировать статью {$post->id}" : 'Добавить статью' }}</h3>
        ...
        <div class="mt-8">
            <form enctype="multipart/form-data" class="space-y-5 mt-5" method="POST" action="{{ isset($post) ? route("admin.posts.update", $post->id) : route("admin.posts.store") }}">
                ...
                @if(isset($post) && $post->thumbnail)
                    <div>
                        <img class="h-52 w-64" src="/storage/posts/{{ $post->thumbnail }}">
                    </div>
                @endif
